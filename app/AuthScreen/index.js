...
const LoginScreen = () => {
  const { login, isError, setIsError } = useContext(AppContext);
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      if (storedMobileNumber) {
        setMobileNumberOrEmpId(storedMobileNumber); // Prefill the mobile number field
      }
      //Line 206 booking seva
  ...
  const validateInput = () => {
    ...
    if (!mobileNumberOrEmpId) {
      setIsError({visible: true, message: 'Mobile number is required'});
      setLoading(false);
      return false;
    }
    if (!pin) {
      setIsError({visible: true, message: 'PIN is required'});
      setLoading(false);
      return false;
    }
    if (pin.length < 4) {
      setIsError({visible: true, message: 'PIN must be at least 4 characters long'});
      setLoading(false);
      return false;
    }
  ...
  const handlePress = async () => {
    setLoading(true);
    setIsError({visible: false, message: ''}); // Clear any previous errors
    ...
    try {
      ...
      if (!selected) {
        setIsError({visible: true, message: 'Invalid company selection'});
        setLoading(false);
        return;
      }
            ...
            <Content>
              <Card>
                ...
                <InputContainer>
                  ...
                  {isError.visible && <ErrorText>{isError.message}</ErrorText>}
